...
for index_snr0 = 1:length(Average_SNR)
    ...
    while(abserr>1e-2)
        gamma0 = gamma0 + 1e-3;
        abserr = abs(1 - integral(@(x)(1/gamma0 - 1./x).*pdfx(x,mu),gamma0,Inf,'AbsTol',1e-3,'RelTol',1e-2)); % integral Introduced in R2012a
    end
...
for index_snr1 = 1:length(Average_SNR)
    ...
    while(accuracy >10^-2)
        ...
        for index_gamma = 1:length(Range_gamma0)
            ...
            E1 = integral(@(x)(1./x).*pdfx(x,mu),gamma0,Inf,'AbsTol',1e-3,'RelTol',1e-2);%E_gamma0{1/gamma} (4.20)
            Pout = integral(@(x)pdfx(x,mu),0,gamma0,'AbsTol',1e-3,'RelTol',1e-2);            
            C_Pout(index_gamma) = log2(1+1/E1)*(1-Pout);
        end
...
%simulation:  Shannon_Capacity_CSIR,Shannon_Capacity_CSITR
for i = 1:Nsample
    gamma_i = exprnd(Average_SNR);
...
title('Figure 4.7:Capacity in Rayleigh Fading')
